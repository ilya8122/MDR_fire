/*
Библиотека изначально скачана с https://github.com/nimaltd/w25qxx
Она была адаптирована для использования с SPL Миландр
В ней есть ряд изъянов:
- Используется "семафор" w25qxx.Lock, но не понятно, как библиотека может работать с SPI из нескольких потоков. Не вижу механизма.
- Задействована функция задержки во многих местах W25qxx_Delay(), которая по умолчанию вызывает HAL_Delay(delay) - т.е. минимум на 1 мс! Убрал.
Непонятна необходимость задержек в принципе (видимо автор расчитывал на многопоточность в некоторых случаях).
- команды заданы жестко в коде
- названия параметров не всегда соответсвует логике команды
- не возвращается результат выполнения основных функций!
*/
#include "w25qxx.h"
...
{
	w25qxx.Lock=1;	
	while(HAL_GetTick()<10)	// Time Delay Before Write Instruction = maximum 10ms
		W25qxx_Delay(1);
  HAL_GPIO_WritePin(_W25QXX_CS_GPIO,_W25QXX_CS_PIN,GPIO_PIN_SET);
  W25qxx_Delay(100);
	uint32_t	id;
